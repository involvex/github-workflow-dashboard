...
async function testWorkflowFunctionality() {
  ...
  try {
    ...
    testStatuses.forEach((status, index) => {
      console.log(`   ✓ Status ${index + 1}: ${status.status}/${status.conclusion || 'null'}`);
    });
    ...
    testDates.forEach((date, index) => {
      ...
      else if (diffMins < 60 * 24) expected = `${Math.floor(diffMins / 60)}h ago`;
    ...
    console.log('   ✓ WorkflowProvider -> RepositorySelectionProvider -> GitHubTokenProvider');
...
// Run the test
testWorkflowFunctionality().then(success => {
  if (success) {
